...
var boss1 = {   //enemy
    ...
    width: 70,
    height: 100, 
    SsposX: Math.floor(Math.random()*1000),
    SsposY: 100,
    posX: 1200, 
    posY: 600,
    velocidade: 0,
    ...
    moveboss1: function(){
        if(boss1.life > 450){
        if(character.posX < this.posX + this.width + 300){
        ...
        if(character.posX > this.posX - 300){
        if(character.posX < this.posX - character.width){
            this.posX -= 3;
    
            }
        }
        else{ if( boss1.posX < 1200){
            boss1.posX +=7;
        }}}
        if(boss1.life < 450){
            if(character.posX < this.posX + this.width + 300){
    ...
    //ataque1 ---> boss1
    Sstorm: function(){
            if(character.vida > 100){
            if(character.posX <= boss1.posX - 300){
                                if(this.SsposY < 600){
                                    this.SsposY += 2;
                    }
                                if(this.SsposY >= 600){
                    ...
                    ctx.fillRect( this.SsposX, this.SsposY, 9, 65);
...
var HBboss1 = { // barra de vida do boss1

    posX: 785,   // posição no eixo x
    posY: 20,   // posição no eixo y
    width: 500, // largura
    height: 20, // altura
    color: "#FF0000",  // cor
...
function char_vida(){   // atualização da barra de vida
 if(boss1.life > 0){
    if(character.posX >= boss1.posX - character.width &&
        barra.width > 0 && character.posX <= boss1.posX + boss1.width
        && character.posY + character.height >= boss1.posY){
        character.vida -= 3;
        barra.width -= 3;

    }
    if(character.posX >= boss1.SsposX - character.width && 
        barra.width > 0 && character.posX <= boss1.SsposX + 9 &&
         character.posY + character.height >= boss1.SsposY &&
         character.posY <= boss1.SsposY + 65){
        character.vida -= 23;
        barra.width -= 23;
    }
 ...
 function boss1_vida(){
    ...
    if(character.posX + character.Wpn_width >= boss1.posX &&
        character.posX + character.Wpn_width && boss1.life > 0 &&
         character.posX - character.Wpn_width <= boss1.posX + boss1.width &&
         character.posY + character.Wpn_height >= boss1.posY){
        HBboss1.width -= 3;
        boss1.life -= 3;
    }
 }
...
function draw (){   // desenhar tela e personagens   
    ...
    if (status_game == game.start) {
        if(boss1.life > 0){
            for(drops = 0; drops < 55; drops +=2.3){
                boss1.Sstorm();
                boss1.SsposX-= 0.8;
                if(boss1.SsposX < 0){
                    boss1.SsposX = 1000
